#!/usr/bin/env python3
"""
同步 運動安全防護資源表.json → 運動安全防護前中後支援資源表.html
用法：python3 更新運動安全防護資源表.py [--open]
"""
...
JSON_PATH = DIR / "運動安全防護資源表.json"
HTML_PATH = DIR / "運動安全防護前中後支援資源表.html"
MARK_BEGIN = "<!-- SAFETY_DATA_BEGIN -->"
MARK_END = "<!-- SAFETY_DATA_END -->"
...
def sync_html(data: dict) -> None:
    ...
    block = (
        f"{MARK_BEGIN}\n"
        f'<script type="application/json" id="safety-data">\n{payload}\n</script>\n'
        f"  {MARK_END}"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="同步運動安全防護 JSON 至 HTML")
